...
!       Obrim els fitxers de llista classificada i mol2 per tal de fer la lectura
		contador=0
          write(*,*)"Escriu el nom del fitxer del qual vulguis treure les 12 components"
	!	   write(*,*)"Exemple: zinc_18207290.mol2"          
          write(*,*)
          write(*,*)
    	 !read(*,*)fitxer
         fitxer='query.mol2'
         inputstat=0
         inputstat2=0
!        open(unit=4,file='fitxer_c.mol2')          
        ...
        call cpu_time(t1) 
!        open(unit=7,file='labels_correct.dat')
...
23        continue
          ...
          if(cabmol.eq."@<TRIPOS>MOLECULE")then
            ...
            nom_mol=cabmol(1:23)      
            ! llegim les dades del mol2!!  (detectar primer @atom?)
            read(4,*)N,Nl
29			continue
            read(4,*)cabmol2
            	if(cabmol2.eq."@<TRIPOS>ATOM")then
                        do i=1,N
!                         write(*,*) "llegim dades mol2" 
       		READ(4,*,iostat=inputstat2)id(I),at(i),x(i),y(i),z(i),orb(i),str(i),str2(i),charge(i)
        		!write(*,*)id(I),at(i),x(i),y(i),z(i),orb(i),str(i),str2(i),charge(i)                       
                        ENDDO
                        read(4,*)cabmol2
                        do ii=1,Nl*2,2
                        read(4,*)inllig(ii),llig1(ii),llig1(ii+1),tipus(ii)
                      !  write(*,*)llig1(ii),llig1(ii+1)                        
                        enddo  	
			...
			!now, read SSIP
		     	...
		     	!n=n+nssip
		     	!do i=n-nssip+1,n
		    	!read(4,*)x(i),y(i),z(i),charge(i)			
			!charge(i)=charge(i)*0.05!/100.
			!enddo
			
        	call stat8(id,at,x,y,z,orb,str,str2,charge,n,nl,inllig,llig1,tipus,pc,nom_mol)

    !            call calc12(x,y,z,n,charge,u11,u12,u13,u14,u15,u16,u21,u22,u23,u24,u25,u26,nom_mol) 
                 
        write(6,'(12(f9.4,2x),a23)')pc(1),pc(2),pc(3),pc(4),pc(5),&
        &pc(6),pc(7),pc(8),pc(9),pc(10),pc(11),pc(12),nom_mol(1:23)
  !  write(6,'(12(f9.4,2x),a)')u11,u12,u13,u14,u15,u16,u21,u22,u23,u24,u25,u26,nom_mol
